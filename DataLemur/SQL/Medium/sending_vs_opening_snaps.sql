...
SELECT 
  age_bucket,
  ROUND(
    SUM(CASE WHEN activity_type = 'send' THEN time_spent ELSE 0 END) * 100.0 / SUM(time_spent), 2
  ) AS send_perc,

  ROUND(
    SUM(CASE WHEN activity_type = 'open' THEN time_spent ELSE 0 END) * 100.0 / SUM(time_spent), 2
  ) AS open_perc
FROM activities
...
SELECT 
  ...
  ROUND(
    ...
    NULLIF(SUM(CASE WHEN activity_type IN ('send', 'open') THEN time_spent ELSE 0 END), 0), 2
  ) AS send_perc,
