...
-- Given a table of transactions and a table of users, write a query to determine if users tend to order more to their primary 
-- address versus other addresses.
...
SELECT 
	100.0 * COUNT(transactions.id) / (SELECT COUNT(*) FROM transactions) AS home_address_percent
FROM transactions
JOIN users
	ON transactions.user_id = users.id
	AND transactions.shipping_address = users.address;